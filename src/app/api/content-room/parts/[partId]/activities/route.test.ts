...
describe("PATCH /api/content-room/parts/:partId/activities", () => {
  ...
  it("rejects unauthenticated (401)", async () => {
    ...
    const res = await handleActivitiesRequest(req({ activity: "editing_youtube", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-1" }) }, {
  ...
  it("rejects without permission (403)", async () => {
    ...
    const res = await handleActivitiesRequest(
      ...
      { params: Promise.resolve({ partId: "CPP-1" }) },
  ...
  it("returns 422 for invalid activity", async () => {
    const deps = makeDeps();
    const res = await handleActivitiesRequest(req({ activity: "invalid_activity", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-1" }) }, deps);
  ...
  it("returns 422 for missing expectedProductVersion", async () => {
    const deps = makeDeps();
    const res = await handleActivitiesRequest(req({ activity: "editing_youtube", isDone: true }), { params: Promise.resolve({ partId: "CPP-1" }) }, deps);
    expect(res.status).toBe(422);
  });
  ...
  it("returns 404 for NOT_FOUND", async () => {
    ...
    const res = await handleActivitiesRequest(req({ activity: "editing_youtube", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-missing" }) }, makeDeps({ repository: repository as never }));
  ...
  it("returns 409 for VERSION_CONFLICT", async () => {
    ...
    const res = await handleActivitiesRequest(req({ activity: "editing_youtube", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-1" }) }, makeDeps({ repository: repository as never }));
  ...
  it("returns 400 for INVALID_TRANSITION (previously_published guard)", async () => {
    ...
    const res = await handleActivitiesRequest(req({ activity: "cover_ready", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-1" }) }, makeDeps({ repository: repository as never }));
  ...
  it("toggles previously_published successfully", async () => {
    ...
    const res = await handleActivitiesRequest(req({ activity: "previously_published", isDone: true, expectedProductVersion: 1 }), { params: Promise.resolve({ partId: "CPP-1" }) }, makeDeps({ repository: repository as never }));
    expect(res.status).toBe(200);
  ...
  it("toggles activity successfully", async () => {
    const deps = makeDeps();
    const res = await handleActivitiesRequest(req({ activity: "editing_youtube", isDone: true, expectedProductVersion: 2 }), { params: Promise.resolve({ partId: "CPP-1" }) }, deps);
    expect(res.status).toBe(200);
  ...
  it("returns 422 for malformed JSON", async () => {
    ...
    const res = await handleActivitiesRequest(request, { params: Promise.resolve({ partId: "CPP-1" }) }, deps);
    expect(res.status).toBe(422);
  });
});
